    ...
    <fwb-tabs v-model="activeTab" variant="underline">
      <fwb-tab name="first" title="Listagem">
        <div class="flex justify-between gap-2">
          <div>
            <FwbInput class="my-1" size="xs" type="text" placeholder="Nome, e-mail, telefone..." />

          </div>
          <div class="flex items-center gap-2">
            <FwbButton class="my-2" @click="showModal = true" color="default" size="sm" pill>
        ...
        <fwb-table>
          ...
          <fwb-table-body>
            <fwb-table-row v-for="usuario in usuarioStore.usuarios" :key="usuario.id">
              <fwb-table-cell><span :class="tagSyle.tagSucces">
                  #{{ usuario.id }}
                </span></fwb-table-cell>
              <fwb-table-cell><span class="bg-blue-900 text-blue-100 p-2 rounded-md">{{ usuario.nome
                  }}</span></fwb-table-cell>
              ...
              <fwb-table-cell class="flex justify-end gap-2">
                <fwb-a href="javascript:void(0)" :class="btnStyle.btnDanger">
                  <iconFA icon="trash" />
                </fwb-a>
                <fwb-a href="javascript:void(0)" :class="btnStyle.btnInfo">
                  <iconFA icon="pencil" />
                </fwb-a>
              </fwb-table-cell>
            </fwb-table-row>
        ...
        <fwb-pagination class="mt-2" v-model="currentPage" next-label="Proxima" previous-label="Anterior"
          :per-page="perPage" :total-items="totalItens"></fwb-pagination>
      </fwb-tab>
...
<script setup>
import { btnStyle } from "@/components/styles/buttons/btnStyle";
import { tagStyle } from "@/components/styles/tags/tagStyle";
