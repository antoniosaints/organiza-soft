<template>
    <TableRow :class="{ 'bg-primary/40 hover:bg-primary/30': hasSelected }" class="transition-colors">
        ...
        <TableCell class="hidden md:table-cell"><span class="px-2 py-1 rounded-md bg-card text-secondary-foreground flex items-center w-min"><User class="w-3 h-3 mr-1" /> {{ formatJustFirstname(data.Cliente?.nome!) }}</span>
        </TableCell>
        <TableCell class="hidden md:table-cell"><span class="px-2 py-1 rounded-md bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200 hover:bg-indigo-300 dark:hover:bg-indigo-700 flex items-center w-min"><CreditCard class="w-3 h-3 mr-1" /> {{ formatFirstLetterUpper(data.metodoPagamento!) }}</span>
        </TableCell>
