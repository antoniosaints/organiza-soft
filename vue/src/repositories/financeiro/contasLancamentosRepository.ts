...
interface IGetAllContasTransacao {
    data: IContaTransacao[];
    total: number;
    pages: number;
}
export default class ContasLancamentosRepository {
    ...
    static async getAll(limit: number, page: number, search?: string | undefined): Promise<IGetAllContasTransacao> {
